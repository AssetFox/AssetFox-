  ...
  <v-layout class="costs-tab-content">
    <v-flex xs12>
      ...
      <div class="costs-data-table">
        <v-data-table :headers="costsGridHeaders" :items="costsGridData"
                      class="elevation-1 fixed-header v-table__overflow"
                      hide-actions>
          <template slot="items" slot-scope="props">
            <td>
              <v-textarea full-width no-resize outline readonly rows="3" v-model="props.item.equation.expression">
                <template slot="append-outer">
                  <v-btn @click="onShowCostEquationEditorDialog(props.item)" class="edit-icon" icon>
            ...
            <td>
              <v-textarea full-width no-resize outline readonly rows="3"
                          v-model="props.item.criterionLibrary.mergedCriteriaExpression">
                <template slot="append-outer">
                  <v-btn @click="onShowCostCriterionLibraryEditorDialog(props.item)" class="edit-icon" icon>
                    <v-icon>fas fa-edit</v-icon>
                  </v-btn>
                </template>
              </v-textarea>
            ...
            <td>
              <v-layout align-start>
                <v-btn @click="onRemoveCost(props.item.id)" class="ara-orange" icon>
    ...
    <CostEquationEditorDialog :dialogData="costEquationEditorDialogData"
                              @submit="onSubmitCostEquationEditorDialogResult"/>

    <CostCriterionLibraryEditorDialog :dialogData="costCriterionLibraryEditorDialogData"
...
export default class CostsTab extends Vue {
  ...
  costEquationEditorDialogData: EquationEditorDialogData = clone(emptyEquationEditorDialogData);
  costCriterionLibraryEditorDialogData: CriterionLibraryEditorDialogData = clone(emptyCriterionLibraryEditorDialogData);
  ...
  onSubmitCostEquationEditorDialogResult(equation: Equation) {
    this.costEquationEditorDialogData = clone(emptyEquationEditorDialogData);

    if (!isNil(equation) && this.selectedCostForEquationOrCriteriaEdit.id !== this.uuidNIL) {
      this.$emit('onModifyCost', setItemPropertyValue('equation', equation, this.selectedCostForEquationOrCriteriaEdit));
    }
  ...
  onSubmitCostCriterionLibraryEditorDialogResult(criterionLibrary: CriterionLibrary) {
    this.costCriterionLibraryEditorDialogData = clone(emptyCriterionLibraryEditorDialogData);

    if (!isNil(criterionLibrary) && this.selectedCostForEquationOrCriteriaEdit.id !== this.uuidNIL) {
      this.$emit('onModifyCost', setItemPropertyValue('criterionLibrary', criterionLibrary, this.selectedCostForEquationOrCriteriaEdit));
    }
