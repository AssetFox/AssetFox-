...
namespace BridgeCare.DataAccessLayer.Inventory
{
    ...
    {   
        ...
        {
            var selectStatement = $"{GetSelectColumnsForPennDotCrosswalk(PennDotCrosswalkRepository.InventoryItems)} FROM PennDot_Report_A WHERE BRIDGE_ID = @value";
        ...
        {
            var selectStatement = $"{GetSelectColumnsForPennDotCrosswalk(PennDotCrosswalkRepository.InventoryItems)} FROM PennDot_Report_A WHERE BRKEY = @value";
        ...
        private InventoryModel GetInventoryModelData<T>(BridgeCareContext db, string selectStatement, T paramValue)
        {
            ...
            if (dataReader.HasRows)
            {
                while (dataReader.Read())
                {
                    ...
                    foreach (InventoryItemModel inventoryItemModel in PennDotCrosswalkRepository.InventoryItems)
                    {
                        if (columnIndex == 0)
                        {
        ...
        private static void AddNbiLoadRatingToModel(InventoryModel model, int brKey, BridgeCareContext db)
        {
            var nbiLoadRatingQuery =
                $"{GetSelectColumnsForPennDotCrosswalk(PennDotCrosswalkRepository.NbiLoadRatingInventoryItems)}" +
            ...
            var nbiLoadRatingInventoryItems = PennDotCrosswalkRepository.NbiLoadRatingInventoryItems;
