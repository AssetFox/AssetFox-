    ...
    <v-layout column>
        <v-flex xs12>
            <v-layout justify-center>
                <v-flex xs3>
                    <v-btn @click='onShowCreateCashFlowRuleLibraryDialog(false)' class='ara-blue-bg white--text'
                           v-show='!hasScenario'>
                        New Library
                    </v-btn>
                                ...
                                <v-data-table :headers='cashFlowRuleGridHeaders' :items='cashFlowRuleGridData'
                                    ...
                                    <template slot='items' slot-scope='props'>
                                        ...
                                        <td>
                                            ...
                                            <v-btn @click='onEditCashFlowRuleCriterionLibrary(props.item)'
                            ...
                            <v-card-text class='cash-flow-library-card'>
                                <v-data-table :headers='cashFlowRuleDistributionGridHeaders'
                                    ...
                                    <template slot='items' slot-scope='props'>
                                        ...
                                        <td>
                                            <v-edit-dialog :return-value.sync='props.item.costCeiling' large lazy
                                                           persistent
                                        ...
                                        <td>
                                            <v-btn @click='onDeleteCashFlowDistributionRule(props.item.id)'
                                                   class='ara-orange'
                                                   icon>
                                                <v-icon>fas fa-trash</v-icon>
                                            </v-btn>
        ...
        <v-flex xs12>
            <v-layout justify-end row v-show='hasSelectedLibrary || hasScenario'>
                <v-btn :disabled='disableSubmitAction() || !hasUnsavedChanges'
...
import { append, clone, find, findIndex, isNil, prepend, propEq, update, reject } from 'ramda';
import {
...
export default class CashFlowEditor extends Vue {
    ...
    @State(state => state.cashFlowRuleModule.scenarioCashFlowRules) stateScenarioCashFlowRules: CashFlowRule[];
    ...
    onSelectedCashFlowRuleLibraryChanged() {
        ...
        if (this.hasScenario) {
            this.cashFlowRuleGridData = this.selectedCashFlowRuleLibrary.cashFlowRules.map((cashFlowRule: CashFlowRule) => ({
                ...cashFlowRule,
                id: getNewGuid(),
                cashFlowDistributionRules: cashFlowRule.cashFlowDistributionRules.map((distributionRule: CashFlowDistributionRule) => ({
                    ...distributionRule,
                    id: getNewGuid(),
                })),
            }));
    ...
    onSelectCashFlowRule() {
        const cashFlowRule: CashFlowRule = find(
            propEq('id', this.cashFlowRuleRadioBtnValue), this.cashFlowRuleGridData,
        ) as CashFlowRule;
    ...
    onSubmitCreateCashFlowRuleLibraryDialogResult(cashFlowRuleLibrary: CashFlowRuleLibrary) {
    ...
    onAddCashFlowRule() {
        ...
        this.cashFlowRuleGridData = prepend(newCashFlowRule, this.cashFlowRuleGridData);
    }

    onDeleteCashFlowRule(cashFlowRuleId: string) {
        this.cashFlowRuleGridData = reject(propEq('id', cashFlowRuleId), this.cashFlowRuleGridData);
    }
    ...
    onAddCashFlowDistributionRule() {
        ...
        this.cashFlowRuleGridData = update(
            findIndex(propEq('id', this.selectedCashFlowRule.id), this.cashFlowRuleGridData),
            {
                ...this.selectedCashFlowRule,
                cashFlowDistributionRules: append(newCashFlowDistributionRule, this.selectedCashFlowRule.cashFlowDistributionRules),
            },
    ...
    onDeleteCashFlowDistributionRule(cashFlowDistributionRuleId: string) {
        this.cashFlowRuleGridData = update(
            findIndex(propEq('id', this.selectedCashFlowRule), this.cashFlowRuleGridData),
            {
                ...this.selectedCashFlowRule,
                cashFlowDistributionRules: reject(propEq('id', cashFlowDistributionRuleId), this.selectedCashFlowRule.cashFlowDistributionRules),
            },
            this.cashFlowRuleGridData,
        );
    }
    ...
    onSubmitCriterionLibraryEditorDialogResult(criterionLibrary: CriterionLibrary) {
        ...
        if (!isNil(criterionLibrary) && this.selectedCashFlowRuleForCriteriaEdit.id !== this.uuidNIL) {
            this.cashFlowRuleGridData = update(
                findIndex(propEq('id', this.selectedCashFlowRuleForCriteriaEdit.id), this.cashFlowRuleGridData),
    ...
    disableSubmitAction() {
        const allDataIsValid = this.cashFlowRuleGridData.every((rule: CashFlowRule) => {
    ...
    onSubmitConfirmDeleteAlertResult(submit: boolean) {
        ...
        if (submit) {
            this.librarySelectItemValue = null;
            this.deleteCashFlowRuleLibraryAction(this.selectedCashFlowRuleLibrary.id);
        }
    }
}
</script>
