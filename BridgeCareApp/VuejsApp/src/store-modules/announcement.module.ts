...
const mutations = {
    ...
    addedOrUpdatedAnnouncementMutator(state: any, announcement: Announcement) {
        state.announcements = any(
            propEq('id', announcement.id),
            state.announcements,
        )
            ? update(
                  findIndex(propEq('id', announcement.id), state.announcements),
                  announcement,
                  state.announcements,
              )
    ...
    deletedAnnouncementMutator(state: any, deletedAnnouncementId: string) {
        if (any(propEq('id', deletedAnnouncementId), state.announcements)) {
            state.announcements = reject(
                (announcement: Announcement) =>
                    announcement.id === deletedAnnouncementId,
                state.announcements,
            );
        }
    },
    sortAnnouncementsMutator(state: any) {
        state.announcements = sortByProperty(
            'createdDate',
            state.announcements,
        );
    },
...
const actions = {
    async getAnnouncements({ commit }: any) {
        await AnnouncementService.getAnnouncements().then(
            (response: AxiosResponse<any[]>) => {
                if (hasValue(response, 'data')) {
                    commit(
                        'announcementsMutator',
    ...
    async upsertAnnouncement({ dispatch, commit }: any, payload: any) {
        await AnnouncementService.upsertAnnouncement(payload.announcement).then(
            (response: AxiosResponse) => {
                if (
                    ...
                    const message: string = any(
                        propEq('id', payload.announcement.id),
                        state.announcements,
                    )
                        ? 'Updated announcement'
                        : 'Added announcement';
                    commit(
                        'addedOrUpdatedAnnouncementMutator',
