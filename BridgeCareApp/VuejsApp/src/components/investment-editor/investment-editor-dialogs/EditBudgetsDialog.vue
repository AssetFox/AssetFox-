                        ...
                        <template slot="items" slot-scope="props">
                            ...
                            <td>
                                <v-edit-dialog
                                    ...
                                    persistent
                                    @save="onEditBudgetName(props.item)"
                                    large
                                    lazy
                                >
                                    <v-text-field
...
import { EditBudgetsDialogData } from '@/shared/models/modals/edit-budgets-dialog';
import {
...
export default class EditBudgetsDialog extends Vue {
    ...
    onAddBudget() {
        const unnamedBudgets = this.editBudgetsDialogGridData.filter(
            (budget: Budget) => budget.name.match(/Unnamed Budget/),
        );
    ...
    onRemoveBudgets() {
        this.editBudgetsDialogGridData = this.editBudgetsDialogGridData.filter(
            (budget: Budget) =>
                !any(propEq('id', budget.id), this.selectedGridRows),
        );
    ...
    onSubmitCriterionLibraryEditorDialogResult(
        ...
        if (
            ...
            this.selectedBudgetForCriteriaEdit.id !== this.uuidNIL
        ) {
            this.editBudgetsDialogGridData = update(
                findIndex(
                    propEq('id', this.selectedBudgetForCriteriaEdit.id),
                    this.editBudgetsDialogGridData,
                ),
                {
                    ...this.selectedBudgetForCriteriaEdit,
                    criterionLibrary: criterionLibrary,
                },
                this.editBudgetsDialogGridData,
            );
    ...
    disableSubmitButton() {
        const allDataIsValid: boolean = this.editBudgetsDialogGridData.every(
            (budget: Budget) => {
                return (
                    this.rules['generalRules'].valueIsNotEmpty(budget.name) ===
                        true &&
                    this.rules['investmentRules'].budgetNameIsUnique(
                        budget,
                        this.editBudgetsDialogGridData,
                    ) === true
                );
