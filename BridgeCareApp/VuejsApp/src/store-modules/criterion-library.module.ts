...
const mutations = {
    criterionLibrariesMutator(
        state: any,
        criteriaLibraries: CriterionLibrary[],
    ) {
    ...
    selectedCriterionLibraryMutator(state: any, libraryId: string) {
        if (any(propEq('id', libraryId), state.criterionLibraries)) {
            state.selectedCriterionLibrary = find(
                propEq('id', libraryId),
                state.criterionLibraries,
            );
        } else {
            state.selectedCriterionLibrary = clone(emptyCriterionLibrary);
        }
    },
    addedOrUpdatedCriterionLibraryMutator(
        state: any,
        library: CriterionLibrary,
    ) {
        state.criterionLibraries = any(
            propEq('id', library.id),
            state.criterionLibraries,
        )
            ? update(
                  findIndex(propEq('id', library.id), state.criterionLibraries),
                  library,
                  state.criterionLibraries,
              )
            : append(library, state.criterionLibraries);
    },
    deletedCriterionLibraryMutator(
        state: any,
        deletedCriteriaLibraryId: string,
    ) {
        state.criterionLibraries = reject(
            propEq('id', deletedCriteriaLibraryId),
            state.criterionLibraries,
        );
    },
    ...
    scenarioRelatedCriterionMutator(state: any, libraryId: string) {
        if (any(propEq('id', libraryId), state.criterionLibraries)) {
            state.scenarioRelatedCriteria = find(
                propEq('id', libraryId),
                state.criterionLibraries,
            );
        } else {
    ...
    upsertScenarioRelatedCriteriaMutator(
        state: any,
        library: CriterionLibrary,
    ) {
...
const actions = {
    ...
    async getCriterionLibraries({ commit }: any) {
        await CriterionLibraryService.getCriterionLibraries().then(
            (response: AxiosResponse) => {
                if (hasValue(response, 'data')) {
                    commit(
                        'criterionLibrariesMutator',
                        response.data as CriterionLibrary[],
                    );
                }
            },
        );
    },
    async upsertCriterionLibrary(
        { commit, dispatch }: any,
        payload: any,
    ): Promise<string> {
        var returningId = getBlankGuid();
        await CriterionLibraryService.upsertCriterionLibrary(
            payload.library,
            ...
            if (
                ...
                const isExistingCriteria: boolean = any(
                    propEq('id', library.id),
                    state.criterionLibraries,
                );
    ...
    async deleteCriterionLibrary({ commit, dispatch }: any, payload: any) {
        await CriterionLibraryService.deleteCriterionLibrary(
            payload.libraryId,
        ).then((response: AxiosResponse) => {
            if (
                hasValue(response, 'status') &&
                ...
                dispatch('addSuccessNotification', {
                    message: 'Deleted criterion library',
                });
                commit('selectedCriterionIsValidMutator', false);
            }
        });
    },
    upsertSelectedScenarioRelatedCriterion(
        { commit, dispatch }: any,
        payload: any,
    ) {
