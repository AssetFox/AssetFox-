        ...
        <v-data-table :headers="consequencesGridHeaders" :items="consequencesGridData"
          ...
          <template slot="items" slot-scope="props">
            <td v-for="header in consequencesGridHeaders">
              ...
              <v-textarea v-if="header.value === 'equation'" full-width no-resize outline readonly rows="3"
                          v-model="props.item.equation.expression">
                <template slot="append-outer">
                  <v-btn @click="onShowConsequenceEquationEditorDialog(props.item)" class="edit-icon" icon>
                    <v-icon>fas fa-edit</v-icon>
                  </v-btn>
                </template>
              </v-textarea>

              <v-textarea v-if="header.value === 'criterionLibrary'" full-width no-resize outline readonly rows="3"
                          v-model="props.item.criterionLibrary.mergedCriteriaExpression">
                <template slot="append-outer">
                  <v-btn @click="onShowConsequenceCriterionLibraryEditorDialog(props.item)" class="edit-icon" icon>
                    <v-icon>fas fa-edit</v-icon>
                  </v-btn>
                </template>
              </v-textarea>
