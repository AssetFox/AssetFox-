<template>
  <v-dialog max-width="450px" persistent v-model="showDialog">
    <v-card>
      ...
      <v-card-text>
        <v-layout column>
          <v-text-field label="Name" outline v-model="newScenario.name"></v-text-field>
          <v-checkbox v-model="shared" label="Share with all?"/>
        </v-layout>
      </v-card-text>
      <v-card-actions>
        <v-layout justify-space-between row>
          <v-btn :disabled="newScenario.name === ''" @click="onSubmit(true)"
...
export default class CreateScenarioDialog extends Vue {
  ...
  onModifyScenarioUserAccess() {
    if (this.showDialog) {
      const currentUser: User = find(propEq('username', getUserName()), this.stateUsers) as User;
