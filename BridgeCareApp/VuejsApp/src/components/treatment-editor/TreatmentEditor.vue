    ...
    <v-layout column>
                                            ...
                                            >
                                                <TreatmentDetailsTab
                                                    ...
                                                    @onModifyTreatmentDetails="
                                                        modifySelectedTreatmentDetails
                                                    "
                                                />
                                            </v-card-text>
                                        </v-card>
                                        ...
                                        <v-card>
                                            ...
                                            >
                                                <CostsTab
                                                    ...
                                                    "
                                                    @onModifyCost="
                                                        modifySelectedTreatmentCost
                                                    "
                                                    @onRemoveCost="
                                                        removeSelectedTreatmentCost
                                                    "
                                                />
                                            </v-card-text>
                                        </v-card>
                                        ...
                                        <v-card>
                                            ...
                                            >
                                                <ConsequencesTab
                                                    ...
                                                    "
                                                    @onModifyConsequence="
                                                        modifySelectedTreatmentConsequence
                                                    "
                                                    @onRemoveConsequence="
                                                        removeSelectedTreatmentConsequence
                                                    "
                                                />
                                            </v-card-text>
                                        </v-card>
        ...
        <v-flex xs12>
            ...
            >
                ...
                <v-btn
                    @click="onUpsertTreatmentLibrary(selectedTreatmentLibrary)"
                    class="ara-blue-bg white--text"
                    v-show="!hasScenario"
                    :disabled="disableCrudButton()"
                >
...
export default class TreatmentEditor extends Vue {
    ...
    onSelectedTreatmentLibraryChanged() {
        ...
        this.hasSelectedLibrary =
            this.selectedTreatmentLibrary.id !== this.uuidNIL;
        if (this.hasSelectedLibrary) {
            this.treatmentSelectItems = this.selectedTreatmentLibrary.treatments.map(
                (treatment: Treatment) => ({
                    text: treatment.name,
                    value: treatment.id,
                }),
        ...
        if (
            ...
            !any(
                propEq('id', this.selectedTreatment.id),
                this.selectedTreatmentLibrary.treatments,
            )
        ) {
    ...
    onStateScenarioSelectableTreatmentsChanged() {
        if (this.currentUrl.indexOf(ScenarioRoutePaths.Treatment) !== -1) {
            this.selectedScenarioTreatments = clone(
                this.stateScenarioSelectableTreatments,
            );
        }
    }
    @Watch('selectedScenarioTreatments')
    onSelectedScenarioTreatmentsChanged() {
        this.treatmentSelectItems = this.selectedScenarioTreatments.map(
            (treatment: Treatment) => ({
                text: treatment.name,
                value: treatment.id,
            }),
    ...
    onTreatmentSelectItemValueChanged() {
        this.hasSelectedLibrary =
            this.selectedTreatmentLibrary.id !== this.uuidNIL;
        if (!this.hasSelectedLibrary) {
            // data coming from selected scenario

            if (hasValue(this.treatmentSelectItemValue)) {
                this.selectedTreatment = find(
                    propEq('id', this.treatmentSelectItemValue),
                    this.selectedScenarioTreatments,
                ) as Treatment;
            } else {
            ...
            if (
                this.treatmentSelectItemValue !== this.selectedTreatment.id &&
                this.selectedTreatment.id !== this.uuidNIL &&
                any(
                    propEq('id', this.selectedTreatment.id),
                    this.selectedTreatmentLibrary.treatments,
            ...
            if (
                hasValue(this.treatmentSelectItemValue) &&
                any(
                    propEq('id', this.treatmentSelectItemValue),
                    this.selectedTreatmentLibrary.treatments,
                )
    ...
    onSelectedTreatmentChanged() {
        ...
        this.selectedTreatmentDetails = {
            ...
            shadowForSameTreatment: this.selectedTreatment
    ...
    onShowCreateTreatmentLibraryDialog(createAsNewLibrary: boolean) {
        if (this.selectedScenarioId != this.uuidNIL) {
            this.createTreatmentLibraryDialogData = {
                showDialog: true,
                selectedTreatmentLibraryTreatments: createAsNewLibrary
                    ? this.selectedScenarioTreatments
                    : [],
            };
        } else {
    ...
    onUpsertTreatmentLibrary(library: TreatmentLibrary) {
        this.createTreatmentLibraryDialogData = clone(
            emptyCreateTreatmentLibraryDialogData,
        );
    ...
    onSaveScenarioTreatment(
        localScenarioSelectedTreatments: Treatment[],
        scenarioId: string,
    ) {
    ...
    modifySelectedTreatmentConsequence(
        modifiedConsequence: TreatmentConsequence,
    ) {
        if (this.hasSelectedTreatment) {
            this.modifySelectedTreatment({
                ...this.selectedTreatment,
                consequences: update(
                    findIndex(
                        propEq('id', modifiedConsequence.id),
                        this.selectedTreatment.consequences,
                    ),
                    modifiedConsequence,
                    this.selectedTreatment.consequences,
    ...
    onSubmitConfirmDeleteAlertResult(submit: boolean) {
        ...
        if (submit) {
            this.librarySelectItemValue = null;
            this.deleteTreatmentLibraryAction({
                libraryId: this.selectedTreatmentLibrary.id,
            });
        }
    }
    ...
    disableCrudButton() {
        this.hasSelectedLibrary =
            this.selectedTreatmentLibrary.id !== this.uuidNIL;
        if (this.hasSelectedLibrary) {
            var validLibData = this.getValidationForSaveButton(
                this.selectedTreatmentLibrary.treatments,
            );
            return (
                !(
                    this.rules['generalRules'].valueIsNotEmpty(
                        this.selectedTreatmentLibrary.name,
                    ) === true
                ) && validLibData
            );
        } else {
            return this.getValidationForSaveButton(
                this.selectedScenarioTreatments,
            );
        }
    }
    getValidationForSaveButton(localTreatments: Treatment[]): boolean {
        const allDataIsValid: boolean = localTreatments.every(
            (treatment: Treatment) => {
                const allSubDataIsValid: boolean = treatment.consequences.every(
                    (consequence: TreatmentConsequence) => {
                        return (
                            ...
                            this.rules[
                                'treatmentRules'
                            ].hasChangeValueOrEquation(
                                consequence.changeValue,
                                consequence.equation.expression,
                            ) === true
                        );
                    },
                );
                ...
                return (
                    ...
                    this.rules['generalRules'].valueIsNotEmpty(
                        treatment.shadowForAnyTreatment,
                    ) === true &&
                    this.rules['generalRules'].valueIsNotEmpty(
                        treatment.shadowForSameTreatment,
                    ) === true
                );
            },
        );
