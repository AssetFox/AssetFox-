...
const mutations = {
    ...
    attributesSelectValuesMutator(
        state: any,
        attributesSelectValues: AttributeSelectValues[],
    ) {
...
const actions = {
    async getAttributes({ commit }: any) {
        await AttributeService.getAttributes().then(
            (response: AxiosResponse<Attribute[]>) => {
                if (hasValue(response, 'data')) {
                    commit('attributesMutator', response.data);
                    commit(
                        'stringAttributesMutator',
    ...
    async getAttributeSelectValues({ commit, dispatch }: any, payload: any) {
        await AttributeService.getAttributeSelectValues(
